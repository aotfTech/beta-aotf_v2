...
If the tag or stack trace mentions a route like `GET /api/v1/posts`:

1. Open the matching file under `app/api/` (App Router maps URL segments to directories)
2. Find the `catch` block that calls `handleApiError(error, "GET /api/v1/posts")` or `reportError()`
