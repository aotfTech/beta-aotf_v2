...
3. `PATCH /api/v1/posts/:postId/applications/:applicationId` with `{ status: "DC", dcDate: "2026-01-15T15:00:00Z" }` (or the equivalent `/jobs/:jobId/applications/:applicationId` route)
...
The provider withdrawal route records the application as `withdrawn`; admins update status through the post- or job-scoped application route.
